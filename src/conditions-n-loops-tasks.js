...
function convertToRomanNumerals(num) {
  ...
  for (let i = 1; i <= 10; i += 1) {
    if (i === lastDigit) {
      lastDigitRoman = roman[i];
      break;
    }
  }
  let romanNumber = '';
  if (num < 10) {
    romanNumber = lastDigitRoman;
  }
  if (num >= 10) {
    romanNumber = `X${lastDigitRoman}`;
  }
  if (num >= 20) {
    romanNumber = `XX${lastDigitRoman}`;
  }
  if (num >= 30) {
    romanNumber = `XXX${lastDigitRoman}`;
  }
  return romanNumber;
}
...
function rotateMatrix(matrix) {
  ...
  const matrixReAssign = matrix;
  for (let i = 0; i < rowCount; i += 1) {
    for (let j = 0; j < colCount; j += 1) {
      matrixReAssign[j][rowCount - 1 - i] = rotatedMatrix[i][j];
    }
  }
  return matrix;
}
